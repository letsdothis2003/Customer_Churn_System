...
class DecisionTree {
    ...
    public void fitFromFile(String filePath) {
        try {
            ...
            for (int i = 0; i < data.size(); i++) {
                ...
                y[i] = Integer.parseInt(row.get(row.size() - 1)); 
            }
    ...
    // New Method: Get Feature Importance
    public double[] getFeatureImportance() {
        double totalImportance = Arrays.stream(featureImportance).sum();
        double[] normalizedImportance = Arrays.copyOf(featureImportance, featureImportance.length);

        if (totalImportance > 0) {
            for (int i = 0; i < normalizedImportance.length; i++) {
                normalizedImportance[i] /= totalImportance;
            }
        }
